...
| 52 | `ops/incident_active.json` (used by `risk_context.py`) blocked by `.gitignore` pattern `ops/*.json` | **Move to `ops/state/incident_active.json`** and adjust `risk_context.py` path. Update `.gitignore` to exclude `ops/state/` from ignore (committed state is OK; logs are not). | Keep audit logs gitignored; allow declarative state files (intent: human-edited indicators) to be committed for reproducibility. |
