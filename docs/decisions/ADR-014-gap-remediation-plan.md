...
| Phase 4 — Dynamic behavior + validator | **In progress** (2 / 5 sub-sections) | `8b20555` → most-recent §4.2 commit (2 commits) | Closed: §4.1 `_load_prometheus_signals` real HTTP queries + 7 mock-based tests + `recent_rollback` cross-source fold; §4.2 `Compute dynamic risk mode` + `Enforce STOP mode` steps in deploy-common.yml; audit step now records base/final mode and signal source. Pending: §4.3 (`validate_agentic --strict` green — requires running the validator and triaging the 18 warnings); §4.4 (make `--strict` blocking in CI); §4.5 (git tags v1.3.0..v1.9.0). |
